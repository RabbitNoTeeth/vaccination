 ...
 * @desc
 */
public class AttentionBabyFragment extends BaseFragment {
    ...
    private AttentionAdapter mBabyAdapter;
    private int page=1;
    private  int rows=15;

    public static AttentionBabyFragment newIntance() {
        return new AttentionBabyFragment();
    }
    @Override
    public void initView(View rootView) {

        mBabyAdapter = new AttentionAdapter(mContext,R.layout.item_article,mDataBeanList);
        ...
        mBabyAdapter.setOnItemClickListener(new MultiItemTypeRecyclerAdapter.OnItemClickListener() {
            @Override
    ...
    private void getList(){
        ...
        params.put("category_id", Global.BABY_ATTENTION);
                        ...
                        if (null != articleBean && null != articleBean.data && null != articleBean.data.data){
                            if (page==1){
                                ...
                                if (articleBean.data.data.size() > 0) {
                                    mBabyAdapter.refresh(articleBean.data.data);
                                } else {
                                ...
                                if (articleBean.data.data.size()>0){
                                    mBabyAdapter.addAll(articleBean.data.data);
                                }else {
